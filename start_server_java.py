...
用法：
  python start_server_java.py        # 前台模式：拉起全部服务后保持运行，Ctrl+C 停止全部
  python start_server_java.py --bg   # 后台模式：拉起全部服务后脚本立即退出（服务独立后台运行）
  建议用项目 venv 解释器运行： venv\\Scripts\\python.exe start_server_java.py

服务清单：
  5002  Java 后端（Spring Boot lettergen-java.jar，/api/*）
  5173  React 前端（vite dev，与 Python 后端共用同一前端）
  8899  静态托管（backend/static-docs，OnlyOffice 加载 docx 用）
  8080  OnlyOffice（Docker 容器 onlyoffice，未运行则自动 docker start）

与 Python 版互斥：
  5002 端口同一时间只能运行一个后端。本脚本启动前会清理 5002 残留
  （若 Python Flask 正在运行会被结束）；Python 版请用 start_server.py。

特性：
  - jar 不存在时自动调用 Maven 构建（内置 .mvn-tools/apache-maven-3.9.9，阿里云镜像）
  - 启动前清理端口残留进程
...
PY = sys.executable
JAVA_DIR = os.path.join(BASE, 'backend', 'java')
JAR = os.path.join(JAVA_DIR, 'target', 'lettergen-java.jar')
MVN = os.path.join(JAVA_DIR, '.mvn-tools', 'apache-maven-3.9.9', 'bin', 'mvn.cmd')
FRONTEND = os.path.join(BASE, 'frontend')
STATIC_DIR = os.path.join(BASE, 'backend', 'static-docs')
JAVA_LOG = os.path.join(JAVA_DIR, 'java_out.log')
...
def build_jar_if_missing():
    """jar 不存在时自动 Maven 构建，返回可用 jar 路径或 None"""
    if os.path.exists(JAR) and os.path.getsize(JAR) > 1000000:
        return JAR
    print('  [Java] 未找到 jar，开始构建（首次需下载依赖，约 1-2 分钟）...')
    if not os.path.exists(MVN):
        print('  [Java] 构建失败：找不到内置 Maven（backend/java/.mvn-tools/apache-maven-3.9.9）')
        return None
    r = subprocess.run([MVN, '-s', 'settings.xml', 'package', '-DskipTests'],
                       cwd=JAVA_DIR, capture_output=True, text=True)
    if r.returncode == 0 and os.path.exists(JAR):
        print('  [Java] 构建成功')
        return JAR
...
def start_java():
    if port_in_use(5002):
        print('  [Java5002] 端口已占用，跳过（注意：若运行的是 Python 后端请先停掉）')
        return
    jar = build_jar_if_missing()
    if not jar:
        return
    logf = open(JAVA_LOG, 'a', encoding='utf-8')
    logf.write('\n==== 启动于 %s ====\n' % time.strftime('%Y-%m-%d %H:%M:%S'))
    subprocess.Popen(
        ['java', '-jar', jar],
        cwd=JAVA_DIR, stdout=logf, stderr=subprocess.STDOUT, creationflags=CREATE_NO_WINDOW)
    print('  [Java5002] 已启动（日志: backend/java/java_out.log）')
...
def main():
    bg = '--bg' in sys.argv[1:]
    print('=' * 54)
    print('  制函服务一键启动（Java 后端）')
    print('=' * 54)

    # 1. 清理端口残留（5002 若被 Python 后端占用会被结束——两后端互斥）
    print('[1/5] 清理端口残留...')
    ...
    start_docker()
    print('[4/5] 启动 Java 后端 / 静态托管 / vite...')
    start_static()
    start_java()
    start_vite()
    ...
    print()
    print('  访问入口: http://127.0.0.1:5173/   （后端: Java lettergen-java.jar）')
    ok = all(results.values())
    print('  状态: %s' % ('全部就绪' if ok else '部分服务未就绪（见上方日志）'))
    print()

    if bg:
        print('后台模式：脚本退出，服务独立运行。停止服务请运行 stop_server_java.py')
        return
