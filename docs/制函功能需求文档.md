...
> 版本：v1.2（2026-09-08）——第 6 章「页面交互说明」全面扩充：每个页面补充界面原型图占位（截图待贴）、界面字段说明表（字段/类型/必填/说明）与字段级业务逻辑；新增「6.4 使用说明页」；同步新增示例模板下载接口（11.2）
> 历史版本：v1.1（2026-09-01）——补充业务框图：新增「4.3 系统整体使用逻辑图」「4.4 系统交互逻辑简览」「8.3/8.4 组件时序图」，第 3 节页面关系图与第 8 节架构/数据流图由 ASCII 统一替换为 mermaid；v1.0（2026-08-31）
...
<img src="assets/44954f60aa510692ce8fc2239fa0c913.png" width="1888" height="852" alt="image">

2. 进入**配置编辑页**：
 
<img src="assets/230cd41779baeea54e3751dbd118768e.png" width="1906" height="813" alt="image">
...
<img src="assets/d5bdbe256c87e3fdc4fb3ff3887f40c9.png" width="1892" height="835" alt="image">

3. 填写**模板名称**（不可与已有模板重名），点击【保存模板配置】；
<img src="assets/cfa343179e090230ba00aa4d1aa8e517.png" width="1850" height="816" alt="image">

5. 系统自动把编辑器当前内容（含标注）保存为该模板的 Word 文件，并跳回列表页。
...
1. 进入**制函页**，下拉选择一个制函模板；
<img src="assets/025a6c276ed6f3855a685a0564caec00.png" width="1896" height="817" alt="image">

3. 上传**批量 Excel**。系统即时校验：
<img src="assets/576290e6db9687e2e99d52e89988c102.png" width="1889" height="812" alt="image">
...
4. 点击【制函】：
<img src="assets/4d2a9e39c569119989cc7be3f51ba660.png" width="1567" height="779" alt="image">
...
> 本章按页面逐一说明：界面原型图（截图待贴）、界面字段说明表、字段级业务逻辑。
> 页面均为单页应用内路由（顶部导航切换），页面**常驻挂载**仅切换可见性——切换路由不丢失编辑状态。

### 6.1 模板列表页（`#/templates`，默认首页）

**界面原型图**：

![模板列表页](images/ui_01_模板列表页.png)

**界面字段说明**：

| 区域 | 字段/元素 | 类型 | 必填 | 说明与业务逻辑 |
|------|-----------|------|------|----------------|
| 顶部 | 【新增模板】按钮 | 按钮 | - | 跳转配置编辑页（新增模式，URL 无 id）；列表页与制函页导航高亮当前页 |
| 模板卡片 | 模板名称 | 文本 | - | 保存时填写，**全系统唯一**（保存接口重名校验：同名且不同 id → 拒绝并提示「模板名称"xxx"已存在」） |
| 模板卡片 | 更新时间 | 文本 | - | 格式 `YYYY-MM-DD HH:MM:SS`；每次保存（新建/修改）自动刷新 |
| 模板卡片 | 包含 Sheet（N） | 文本 | - | 该模板配置的 Excel 中全部 Sheet 名称清单（顿号分隔）+ 数量；来自保存模板时解析上传 Excel 的结果 |
| 模板卡片 | 【✏️ 修改】按钮 | 按钮 | - | 跳转配置编辑页（编辑模式，URL 带 `?id=tpl_xxx`）：编辑器自动加载该模板已保存的 Word，右侧回显 Sheet 清单与表格样式配置 |
| 模板卡片 | 【🗑 删除】按钮 | 按钮 | - | **二次确认**（confirm 弹窗：确认删除模板配置「xxx」？该操作不可恢复）→ 删除 index 条目 + 模板目录（word.docx/excel.xlsx 一并删除）→ 刷新列表 |
| 空状态 | 引导文案 | 文本 | - | 无任何模板时显示：`还没有模板配置，点击右上角「新增模板」上传 Word 模板并标注 Sheet 插入位置` |
| 底部 | 说明行 | 文本 | - | 固定提示：模板配置 = Word 模板（含 Sheet 插入位置标注）+ Excel 的 Sheet 结构定义 |

**业务逻辑**：

- 数据来源 `GET /api/templates`（index.json 全量清单）；
- **自动刷新**：本页常驻挂载，监听 hashchange——从配置页/制函页切回本页时自动重新拉取，保证保存/删除后数据最新；
- 删除是物理删除（不可恢复），删除后该模板在制函页不可再选；已用该模板生成的历史函证文件不受影响。

### 6.2 配置编辑页（`#/config`，带 `?id=` 为修改模式）

**界面原型图**：

![配置编辑页整体布局](images/ui_02a_配置编辑页整体.png)
>
![占位段插入效果](images/ui_02b_占位段插入效果.png)
>
![表格样式高级区](images/ui_02c_表格样式高级区.png)

**界面字段说明（右侧操作面板）**：

| 区域 | 字段/元素 | 类型 | 必填 | 说明与业务逻辑 |
|------|-----------|------|------|----------------|
| 1. 上传 Word | 模板文件选择 | 文件（.docx） | 新建必填* | 点击选择 Word 文件；上传后：① 保存为工作区 `current.docx`（此后编辑器加载与 forcesave 回写均基于它）② 编辑器加载其内容。**编辑模式未重新上传时沿用该模板已保存的 Word** |
| 1. 上传 Word | 📥 下载示例 Word 模板 | 链接 | - | 未上传 Word 时显示；下载 `函证模板示例.docx`（接口 `GET /api/templates/sample/word`），修改内容后可直接上传使用 |
| 1. 上传 Word | 模板信息（tplInfo） | 文本 | - | 上传成功后显示文件名/页数等解析信息；作为 Word 是否已就绪的视觉状态 |
| 2. 上传 Excel | Excel 文件选择 | 文件（.xlsx/.xlsm） | 新建必填 | 上传后即时解析全部 Sheet（名称/表头/行数），驱动「3. 可用 Sheet」列表；**Excel 允许没有数据行**——它只定义 Sheet 结构；上传接口同时对批量格式做校验拦截（详见 6.3） |
| 3. 可用 Sheet | Sheet 项（名称 + N 行数据 + 表头预览） | 列表 | - | 每项可点击：在编辑器**当前光标位置**插入该 Sheet 的占位段（一行蓝色加粗文字）；插入后该项标 ✅ 已插入，可重复点击在多处插入 |
| 保存区 | 模板名称 | 文本输入 | ✓ | 保存必填；**不可与已有模板重名**（后端校验，重名 → 弹窗提示） |
| 保存区 | 表格样式（高级，可选） | 折叠区 | - | 默认收起；六项均带默认值（不改则与现状渲染一致）：**字号**（默认 10 磅）/ **字体**（默认继承模板，可选宋体/仿宋/黑体等）/ **水平对齐**（居中）/ **垂直对齐**（顶端，可选居中）/ **行高**（自动，可选最小值 20 磅——内容多时自动加高不截断）/ **列宽**（三档：均分 / 按内容自适应 / 自定义比例——比例按 Sheet 逐个配置如 `2:1:1`，未配置的 Sheet 回落均分） |
| 保存区 | 【保存模板配置】/【保存修改】 | 按钮 | - | 保存中锁定防重复点击；点击后系统**自动触发 OnlyOffice 强制保存**把编辑器当前内容（含占位段标注）回写到后端，轮询确认完成后落盘为模板配置（word.docx + excel.xlsx + style），成功后提示并可回列表页使用 |
| 顶部 | 模式标识 | 文本 | - | `✏️ 编辑模板「xxx」`（带 id）/ `📄 新增模板`；新增模式**编辑器空白态**：不加载历史文档，显示「请先在右侧上传 Word 模板」占位 |

**业务逻辑**：

- **占位段即插入定义**：占位段文字 `【Sheet「Sheet名」表格将在此处展示】` 由系统生成（用户不手写）；制函时按占位段自动提取插入关系（模板即定义），同一 Sheet 多处插入会生成多条绑定；
- **保存链路（forcesave 回写）**：保存 → 后端向 OnlyOffice 发 forcesave 命令 → OO 把当前文档 POST 回 `/api/oo/callback` → 后端覆盖 `current.docx` → 前端轮询 `save_status` 确认 → 复制为模板 `word.docx`；**切换模板上传新 Word 时，旧编辑器的延迟回写会被按 document.key 校验跳过**（防止旧文档覆盖新模板）；
- **样式持久化**：表格样式存入模板配置的 `style` 字段，制函渲染时应用；仅当保存请求携带 style 时才更新（旧前端不携带不会覆盖已有配置），脏字段自动回落默认值；
- **协作名**：编辑器配置了默认协作用户（审计员），不再弹出「输入用于协作的名称」对话框。

### 6.3 Excel 制函页（`#/generate`）

**界面原型图**：

![制函页布局](images/ui_03a_制函页布局.png)
>
![制函页匹配结果](images/ui_03b_制函页匹配结果.png)

**界面字段说明**：

| 区域 | 字段/元素 | 类型 | 必填 | 说明与业务逻辑 |
|------|-----------|------|------|----------------|
| 1. 选择制函模板 | 模板下拉 | 下拉单选 | ✓ | 选项 = 全部模板配置（含 Sheet 数提示）；未选模板时上传 Excel 控件禁用；选中后显示该模板包含的 Sheet 清单，并提示"模板即定义"（插入关系由模板占位段自动提取） |
| 2. 上传 Excel | Excel 文件选择 | 文件（.xlsx/.xlsm） | ✓ | **必须先选模板**（否则弹窗提示）；上传即校验（见下），通过后落盘并展示匹配结果 |
| 2. 上传 Excel | 📥 下载数据导入模板 | 链接 | - | 下载 `批量制函数据导入模板.xlsx`（接口 `GET /api/templates/sample/excel`）：内含 3 个示例 Sheet（科目余额分录/非标事项/票据事项），表头前两列为分组列、其余列为示例数据，示例单位组合为系统已登记函证（可直接体验制函）；按模板填入实际数据后上传 |
| 2. 上传 Excel | 已加载信息 | 文本 | - | 显示 `已加载：<文件名> · N 封函证`；上传失败显示失败原因 |
| 2. 上传 Excel | 匹配结果表格 | 表格 | - | 列：**被审计单位** / **被询证单位** / **函证编号**（未匹配显示红色"未匹配"）/ **状态**（`✓ 已匹配` 或红色 `⚠ 系统没有对应的函证，制函将跳过`）；未匹配行整行浅红底；存在未匹配时表格下方红色汇总数量。数据来源：单位组合与函证登记数据（现阶段 `data/letters_registry.json` 模拟）精确匹配 |
| 3. 制函 | 【制函（N 封）】按钮 | 按钮 | - | 文案实时显示将生成的函证数（匹配成功数）；**禁用条件**：未选模板 / 未上传 Excel / 生成中 / **全部组合均未匹配**（此时显示红色指引"请先在函证系统完成登记后重新上传"）；生成中锁定防重复点击 |
| 底部 | 状态行 | 文本 | - | 全程反馈：选模板确认 → Excel 校验结果 → 制函中 → `✓ 制函成功！共 N 封函证，ZIP 已开始下载（未生成表格的 Sheet：…若有）` |

**业务逻辑**：

- **上传即校验（三道拦截，均弹窗阻断）**：
  1. **格式校验**：各 Sheet 表头前两列必须为「被审计单位」「被询证单位」，否则拒绝（"请上传批量格式 Excel"）；
  2. **空分组行拦截**：分组列任一为空 → 拒绝并精确提示位置（`科目余额分录 第5行（被审计单位）`，超 20 处汇总总数）；
  3. **函证编号匹配**：组合去匹配函证登记数据，未匹配行红色展示；
- **制函执行规则**：仅生成**匹配成功**的函证，未匹配组合整封跳过并在结果中提示；全部未匹配则终止；对每封函证：以模板 Word 为底稿，把每个占位段替换为该组合对应的数据表格（**前两列分组列不进入表格**，表格从第 3 列开始）；某 Sheet 该组合无数据行或 Excel 中不存在该 Sheet 时，对应占位段**自动删除**（不残留提示文字）；全部函证打包 ZIP 自动下载；
- **表格样式**：应用模板配置的 style（见 6.2 表格样式），未配置时使用内置默认。

### 6.4 使用说明页（`#/help`）

**界面原型图**：

![使用说明页](images/ui_04_使用说明页.png)

**界面字段说明**：

| 区域 | 字段/元素 | 类型 | 说明与业务逻辑 |
|------|-----------|------|----------------|
| 🎯 解决什么问题 | 场景说明 | 文本 | 手工复制粘贴 Excel 数据到 Word 函证的痛点 → 两步自动化（Word 定版式标注位置 / Excel 供数据一键批量生成）；适用范围提示（仅往来函证） |
| 📝 第一步 | 配置流程（有序列表） | 文本+链接 | 新增模板 → 上传 Word（含 <a> 下载示例 Word 模板</a>）→ 上传 Excel（可无数据）→ 光标处插占位段 → 填名保存；修改方式说明 |
| 🚀 第二步 | 制函流程（有序列表） | 文本+链接 | 选模板 → 上传批量 Excel（含 <a> 下载数据导入模板</a>；前两列规则/第 3 列起进表格/多 Sheet 归集说明）→ 即时校验与匹配说明 → 制函 ZIP 下载 |
| 📌 关键规则速查 | 规则表 | 表格 | 占位段替换与自动删除 / 函证编号匹配与跳过 / 表格样式默认值与高级配置 / 模板即定义 |
| ✅ 建议的上手路径 | 引导 | 文本 | 下载两个示例 → 全流程走通约 5 分钟 |

**业务逻辑**：纯静态说明页（无接口依赖）；两个下载链接与 6.2/6.3 内的下载入口指向同一接口。
...
| `/api/templates/sample/word` | GET | 下载示例 Word 模板（`函证模板示例.docx`，来自 static-docs/demo_template.docx）。入口：配置编辑页上传区 |
| `/api/templates/sample/excel` | GET | 下载批量制函数据导入模板（动态生成：3 个示例 Sheet，前两列分组列 + 示例数据，示例单位为已登记函证）。入口：制函页与使用说明页 |
